...
export async function consultarCA(numeroCA) {
    ...
    try {
        console.log(`[CAEPI] Searching web for CA: ${numeroCA}`);

        // This function now signals that web search is needed
        // The actual search will be performed by gemini.js layer which has access to search_web

        return {
            status: 'REQUIRES_WEB_SEARCH',
            ca_number: numeroCA,
            search_query: `CA ${numeroCA} EPI certificado aprovação descrição técnica produto`,
            message: 'Web search required - will be handled by AI layer'
        };

    } catch (error) {
        console.error('[CAEPI] Web search setup failed:', error);
        return {
