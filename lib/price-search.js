...
export async function buscarMelhoresPrecos({ query, has_ca, ca_numero, ca_descricao_tecnica, ca_nome_comercial, query_semantica }) {
    ...
    try {
        ...
        if (caQuery) {
            ...
            // SECURITY CHECK: Google often ignores the number and returns generic items.
            // We must STRICTLY filter items that actually contain the CA number in the title.
            rawResults = googleResults.filter(item => {
                // Check if CA number exists in title (e.g. "40377")
                return item.titulo && item.titulo.includes(ca_numero);
            });

            if (rawResults && rawResults.length > 0) {
                console.log(`[PRICE-SEARCH] ✅ Attempt 1 Success! Found ${rawResults.length} VERIFIED items containing CA ${ca_numero}.`);
                strategyUsed = 'exact_ca_match';
                finalQuery = caQuery;
                isExactCaStrategy = true;
            } else {
                console.log(`[PRICE-SEARCH] ⚠️ Attempt 1 yielded results, but NONE contained the CA number "${ca_numero}". Discarded all.`);
                rawResults = []; // Force empty to trigger "Not Found" or Fallback (if enabled)
            }
        }
