    ...
    if (ca_descricao_tecnica && ca_descricao_tecnica.length > 50) {
        try {
            ...
            if (intelligentResult && intelligentResult.query) {
                // VALIDATION: If extracted query is too generic (no category/model) AND we have a semantic query, use semantic
                // This prevents generic "Produto preco Brasil" searches for unidentified items (like some EPIs)
                if ((!intelligentResult.specs.model && !intelligentResult.specs.category) && query_semantica) {
                    console.log('[PRICE-SEARCH] Intelligent Search output generic. FALLBACK to Semantic Query.', query_semantica);
                    finalQuery = query_semantica;
                    origin = 'semantic_over_intelligent_generic';

                    // Add generic keywords to at least filter something if possible, or leave empty to rely on full query
                    if (query_semantica) {
                        relevanceKeywords.push(...query_semantica.split(' ').slice(0, 3)); // Use first 3 words of semantic query as filter baseline
                    }
                } else {
                ...
                // FALLBACK: If intelligent search returns nothing, PREFER semantic query over ca_nome_comercial
                // This is crucial because ca_nome_comercial often defaults to "Produto" if not found
                if (query_semantica) {
                    finalQuery = query_semantica;
                    origin = 'intelligent_fallback_semantic';

                    // CRITICAL: Populate relevanceKeywords with semantic query words to enable filtering
                    relevanceKeywords.push(...query_semantica.split(' ').slice(0, 3).map(w => w.toLowerCase()));
                } else {
                    finalQuery = query || ca_nome_comercial;
                    origin = 'intelligent_fallback_legacy';
                }
            }
        } catch (error) {
            console.error('[PRICE-SEARCH] Intelligent Search failed, falling back:', error.message);

            // ERROR FALLBACK: Prefer semantic query
            if (query_semantica) {
                finalQuery = query_semantica;
                origin = 'intelligent_error_fallback_semantic';
                relevanceKeywords.push(...query_semantica.split(' ').slice(0, 3).map(w => w.toLowerCase()));
            } else {
                finalQuery = query || ca_nome_comercial;
                origin = 'intelligent_error_fallback_legacy';
            }
        }
    }
    else if (query) {
