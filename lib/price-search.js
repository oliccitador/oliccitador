...
export async function buscarMelhoresPrecos({ query, has_ca, ca_numero, ca_descricao_tecnica, ca_nome_comercial, query_semantica }) {
    ...
    // STRATEGY: Intelligent Search (if description available)
    ...
    // If ML has few results (< 3), try Google Shopping
    if (rawResults.length < 3) {
        console.log('[PRICE-SEARCH] Few results on ML, searching Google Shopping...');
        try {
            const googleResults = await searchGoogleShopping(finalQuery);
            if (googleResults.length > 0) {
                console.log(`[PRICE-SEARCH] Google Shopping returned ${googleResults.length} results`);
                // Merge results, prioritizing ML but adding Google results
                rawResults = [...rawResults, ...googleResults];
            }
        } catch (err) {
            console.error('[PRICE-SEARCH] Google Shopping error:', err.message);
        }
    }
