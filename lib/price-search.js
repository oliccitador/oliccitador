...
export async function buscarMelhoresPrecos({ has_ca, ca_numero, ca_descricao_tecnica, query_semantica }) {
    ...
    if (has_ca && ca_numero) {
        ...
        // Truncate description to avoid too long queries (Bing limit)
        const shortDesc = ca_descricao_tecnica ? ca_descricao_tecnica.substring(0, 60) : '';
        finalQuery = `CA ${ca_numero} ${shortDesc}`;
