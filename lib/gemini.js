...
export async function analyzeWithFlow(description, ca, catmat) {
  ...
  try {
    ...
    try {
      ...
      // Helper to clean description and extract meaningful words
      ...
      if (jsonMatch) {
        ...
        if (geminiAnalysis.query_semantica_limpa) {
          // If AI returned a short query, try to enrich it from description
          if (geminiAnalysis.query_semantica_limpa.split(' ').length < 10) {
      ...
      // Helper defined inside catch for scope safety in fallback
      const cleanAndExtract = (text, wordLimit) => {
        if (!text) return "";
        let cleaned = text
          .replace(/\bUN\b\s+[\d,\.]+/gi, '')
