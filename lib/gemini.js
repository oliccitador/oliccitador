  ...
  try {
    ...
    try {
      ...
      if (geminiError.message === 'GEMINI_TIMEOUT') {
        ...
        // Improved fallback: Clean description better
        let cleanedDesc = description
          // Remove billing data patterns (UN, numbers with commas/dots)
          .replace(/\bUN\b\s+[\d,\.]+/gi, '')
          .replace(/\b[\d,\.]+\s+[\d,\.]+\s+[\d,\.]+/g, '')
        ...
        // Extract meaningful words (filter short words)
        const words = cleanedDesc.split(' ').filter(w => w.length > 2);
        const productName = words.slice(0, 25).join(' ');  // 25 words for complete query

        geminiAnalysis = {
          regra_edital_gemeo: words.slice(0, 15).join(' '),  // 15 words for snippet
          query_semantica_limpa: productName,
