...
export function determineFlow({ description, ca, catmat }) {
    ...
    const hasCATMAT = !!detectedCatmat && detectedCatmat.trim().length > 0;

    // Flow 1: TR + CA (EPI → CAEPI Direct)
    if (hasCA && !hasCATMAT) {
        return { flow: 'FLOW_1_CA', ca, catmat: null };
    }

    // Flow 3: TR + CATMAT (CATMAT isolated + PNCP isolated)
    // Now also catches auto-detected codes from step 1
    if (hasCATMAT && !hasCA) {
        return { flow: 'FLOW_3_CATMAT', ca: null, catmat: detectedCatmat };
    }

    // Flow 2: TR without CA or CATMAT (PNCP literal → semantic)
    if (!hasCA && !hasCATMAT) {
        return { flow: 'FLOW_2_PNCP', ca: null, catmat: null };
    }
 ...
 */
export async function executeFlow(flowInfo, input) {
    const { description } = input;
    const { flow, ca, catmat } = flowInfo;
...
async function executeFlow1(description, ca) {
    ...
    return {
        ...
        descricao_tecnica_limpa: especificacaoLimpa, // Alias for frontend consistency
        codigo_utilizado: ca,
...
async function executeFlow2(description) {
    ...
    // REGRA ABSOLUTA: query_semantica_limpa NUNCA pode ser vazia
    let querySemanticaLimpa = 'SEM_CORRESPONDENCIA';

    if (melhorDescricao && melhorDescricao.descricao) {
        querySemanticaLimpa = melhorDescricao.descricao;
    } else if (semanticResults && semanticResults.length > 0) {
        // Fallback para o primeiro resultado semântico se não houver melhor descrição consolidada
        querySemanticaLimpa = semanticResults[0].descricao;
    } else {
        // Se tudo falhar, usar a própria descrição original limpa como fallback final
        // Regex ajustado para permitir caracteres acentuados (PT-BR)
        querySemanticaLimpa = description.replace(/[^a-zA-Z0-9\sÀ-ÿ]/gi, ' ').replace(/\s+/g, ' ').trim().substring(0, 100);
    }
    ...
    if (!melhorDescricao) {
        return {
            ...
            query_semantica_limpa: querySemanticaLimpa, // Garantido não ser null
            descricao_tecnica_limpa: querySemanticaLimpa, // Alias fallback
    ...
    return {
        ...
        query_semantica_limpa: querySemanticaLimpa,
        descricao_tecnica_limpa: querySemanticaLimpa, // Alias for frontend consistency (uses semantic query as clean desc in this flow)
        bloco_justificativa: blocoJustificativa,
...
async function executeFlow3(description, catmat) {
    ...
    return {
        ...
        descricao_tecnica_limpa: descricaoConsolidada, // Alias for frontend consistency
        codigo_utilizado: catmat,
        bloco_justificativa: blocoJustificativa,
        catmat_data: catmatData,
