...
export async function POST(request: NextRequest) {
    try {
        ...
        // ✅ PERSISTIR NO DB (OPCIONAL / RESILIENTE)
        // Se falhar (ex: sem DATABASE_URL no preview), apenas loga e segue.
        try {
            ...
            // Marca como persistido
            result.persisted = true;

        } catch (dbError: any) {
            console.warn('⚠️ AVISO: Falha ao persistir no banco de dados (ignorando para retornar análise):', dbError.message);
            // Não quebra o fluxo, apenas marca como não persistido
            result.persisted = false;
            result.persistence_error = dbError.message;
        }

        // ✅ Retornar resultado completo (com ou sem DB)
